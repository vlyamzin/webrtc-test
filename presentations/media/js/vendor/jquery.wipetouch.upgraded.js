// jQuery WipeTouch - Fork based on 1.2.0 (plus remove event listener support)
// ------------------------------------------------------------------------
//
...
// http://devv.com
//
// Remove event listener support added by Philip Bulley
// http://milkisevil.com
//
// USAGE
// ------------------------------------------------------------------------
...
(function ($)
{
	$.fn.wipetouch = function (settings)
	{
		...
		this.each(function ()
		{
			if( settings.remove )
			{
				// Remove all events for the specified DOM elements
				$(this).bind("touchstart", this.onTouchStart);
				$(this).unbind("touchend", this.onTouchEnd);
				$(this).unbind("touchmove", this.onTouchMove);

				$(this).unbind("mousedown", this.onTouchStart);
				$(this).unbind("mouseout", this.onTouchEnd);
				$(this).unbind("mousemove", this.onTouchMove);
				$(this).unbind("mouseup", this.onTouchEnd);
			...
			this.onTouchStart = function(e)
			{
				...
				if (!isMoving && start)
				{
					...
					if (useMouseEvents)
					{
						...
						$(this).one("mouseup", this.onTouchEnd);
					}
					else
					{
			...
			else
			{
				...
				$(this).bind("mouseout", this.onTouchEnd);
			}
		});
